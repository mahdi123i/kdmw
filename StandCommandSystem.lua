--[[
	ROBLOX STAND COMMAND SYSTEM v6.1 — DA HOOD PATCH (ENHANCED)
	GitHub-Loadable | Production-Ready | Executor-Compatible
	
	V6.1 ENHANCEMENTS:
	✔ Chat command listener (legacy + modern TextChatService)
	✔ Owner auto-init sequence with character reset
	✔ Stand follow/hover visual with smooth lerp
	✔ gkill! dominant state (no jitter, clean execution)
	✔ Auto-reload system (config-based)
	✔ Visual debug feedback (minimal, non-spam)
	
	Usage: loadstring(game:HttpGet("RAW_GITHUB_URL"))()
	
	Configuration via getgenv():
	- getgenv().Owner = "YourUsername"
	- getgenv()._C = { CrewID = 1, StandMode = "auto", AutoReload = true, Position = "safe1" }
...
Config.CustomPrefix = getgenv().CustomPrefix or "."
...
-- ============================================================================
-- KILL ENGINE (gkill!) — DA HOOD REAL EXECUTION (FIX #4: DOMINANT STATE)
-- ============================================================================
...
-- ============================================================================
-- OWNER SYSTEM (FIX #2: AUTO-INIT SEQUENCE)
-- ============================================================================
...
function OwnerSystem:autoInit()
	...
	task.spawn(function()
		...
		print("[StandSystem] Owner auto-init complete. Stand summoned at " .. Config.Position)
	end)
end

-- ============================================================================
-- AUTO RELOAD SYSTEM (FIX #5: CONFIG-BASED)
-- ============================================================================
...
-- ============================================================================
-- STAND HOVER ENGINE (FIX #3: STAND FOLLOW / HOVER VISUAL)
-- ============================================================================
...
function StandHoverEngine:start()
	...
	self.hoverConnection = RunService.Heartbeat:Connect(function()
		...
		-- Stop hovering during KILL state (stand snaps to target)
		if StateManager:isState(StateManager.states.KILL) then
			return
		end
...
function StandHoverEngine:executeHover()
	...
	-- V6 behavior: Stand positioned behind and slightly to the side
...
-- ============================================================================
-- CHAT HOOK (FIX #1: CHAT COMMAND LISTENER)
-- ============================================================================

local CustomPrefix = getgenv().CustomPrefix or "."
local chatListenerActive = false

local function setupChatHook()
	if chatListenerActive then
		return
	end
	...
	-- Legacy chat system (Chatted event)
	pcall(function()
		player.Chatted:Connect(function(message)
			if string.sub(message, 1, 1) == CustomPrefix or string.sub(message, 1, 1) == "." then
				CommandDispatcher:execute(message)
			end
		end)
	end)
	
	-- New TextChatService (modern chat)
	pcall(function()
			...
			if chatChannel then
				chatChannel.MessageReceived:Connect(function(message)
					if message.TextSource and message.TextSource.Parent == player then
						...
						if string.sub(text, 1, 1) == CustomPrefix or string.sub(text, 1, 1) == "." then
	...
	print("[StandSystem] Chat listener activated. Prefix: " .. CustomPrefix)
end
...
local function mainLoop()
	mainLoopConnection = RunService.Heartbeat:Connect(function()
		...
		-- FIX #3: Stand hover loop when active
...
local function init()
	...
	print("[StandSystem] v6.1 INITIALIZED - All systems active")
	print("[StandSystem] Owner: " .. Config.Owner .. " | Position: " .. Config.Position .. " | Crew: " .. Config.CrewID)
	print("[StandSystem] AutoReload: " .. tostring(Config.AutoReload) .. " | Prefix: " .. CustomPrefix)
end
