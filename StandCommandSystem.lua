--[[
	ROBLOX STAND COMMAND SYSTEM v6.0 REBUILD
	Professional Exploit Loader | Da Hood Compatible
	
	CORE FEATURES:
	✔ Loader-like boot sequence (forced reset → teleport → crew → summon)
	✔ Chat command listener (legacy + TextChatService, owner-only)
	✔ Owner-only control with stand follow/hover
	✔ gkill! dominant execution (state-locked, clean)
	✔ Auto-reload system (silent recovery)
	✔ Da Hood safe design (real remotes, rate-limited)
	✔ Silent operation (minimal prints)
	
	Usage: loadstring(game:HttpGet("RAW_URL"))()
	
	Configuration:
	- getgenv().Owner = "YourUsername"
	- getgenv()._C = { CrewID=35638587, AutoReload=true, Position="safe1", CustomPrefix="." }
]]
...
Config.CustomPrefix = (getgenv()._C and getgenv()._C.CustomPrefix) or getgenv().CustomPrefix or "."
...
-- ============================================================================
-- KILL ENGINE (gkill!) — DOMINANT STATE EXECUTION
-- ============================================================================
...
-- ============================================================================
-- STAND HOVER ENGINE (SMOOTH FOLLOW BEHAVIOR)
-- ============================================================================
...
-- ============================================================================
-- CHAT LISTENER (OWNER-ONLY, DUAL SYSTEM)
-- ============================================================================

local ChatListener = {}
ChatListener.active = false

function ChatListener:setup()
	if self.active then
		return
	end
